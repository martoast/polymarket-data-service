    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-14 transition-all duration-700 delay-100"
            ...
            <div class="flex items-center gap-3 mb-6">
                <div class="w-8 h-8 rounded-xl bg-[#0093fd]/10 border border-[#0093fd]/20 flex items-center justify-center">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                @foreach([
                    ['BTC', 'Bitcoin',  'eth/btc-usd'],
                    ['ETH', 'Ethereum', 'eth/eth-usd'],
                    ['SOL', 'Solana',   'eth/sol-usd'],
                ] as [$sym, $name, $feed])
                <div class="bg-[#17181c] border border-[#1f2937] hover:border-[#0093fd]/20 rounded-2xl p-5 transition-colors">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2.5">
                            <div class="w-8 h-8 rounded-lg bg-[#0093fd]/10 border border-[#0093fd]/15 flex items-center justify-center font-bold text-xs text-[#0093fd]">
                                {{ $sym }}
                            </div>
                            <div>
        ...
        <div class="transition-all duration-700 delay-200"
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                @php
                $stations = [
                    ['RJTT', 'Tokyo',        'JP', 'Asia/Tokyo',          'celsius'],
                    ['EGLL', 'London',        'GB', 'Europe/London',       'celsius'],
                    ['LFPG', 'Paris',         'FR', 'Europe/Paris',        'celsius'],
                    ['WSSS', 'Singapore',     'SG', 'Asia/Singapore',      'celsius'],
                    ['RKSI', 'Seoul',         'KR', 'Asia/Seoul',          'celsius'],
                    ['ZBAA', 'Beijing',       'CN', 'Asia/Shanghai',       'celsius'],
                    ['KORD', 'Chicago',       'US', 'America/Chicago',     'fahrenheit'],
                    ['KJFK', 'New York',      'US', 'America/New_York',    'fahrenheit'],
                    ['KLAX', 'Los Angeles',   'US', 'America/Los_Angeles', 'fahrenheit'],
                    ['KMIA', 'Miami',         'US', 'America/New_York',    'fahrenheit'],
                ];
                @endphp
                @foreach($stations as [$icao, $city, $country, $tz, $unit])
                <div class="bg-[#17181c] border border-[#1f2937] hover:border-[#0093fd]/20 rounded-xl p-4 transition-colors">
                    <div class="flex items-center justify-between mb-2.5">
                        <span class="font-mono text-xs font-bold text-[#0093fd]">{{ $icao }}</span>
                    ...
                    <span class="text-[9px] font-mono font-bold bg-[#0093fd]/10 border border-[#0093fd]/15 text-[#0093fd] px-1.5 py-0.5 rounded">
                        {{ $unit === 'fahrenheit' ? '°F' : '°C' }}
                    </span>
                </div>
                @endforeach
            ...
            {{-- Data sources note --}}
            <div class="mt-5 bg-[#17181c] border border-[#1f2937] rounded-xl p-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                @foreach([
                ...
                <div>
                    <div class="text-[9px] font-bold uppercase tracking-wider text-[#0093fd]/60 mb-1">{{ $k }}</div>
                    <div class="text-xs text-[#697d91] leading-relaxed">{{ $v }}</div>
                </div>
                @endforeach
            </div>
        </div>
...
<section class="py-20 border-b border-[#1f2937]"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-[#17181c]/50 border border-[#1f2937] hover:border-[#2e3841] rounded-2xl p-6 transition-all duration-500"
                 :style="vis ? 'opacity:1;transform:translateY(0);transition-delay:{{ $i * 80 }}ms' : 'opacity:0;transform:translateY(20px)'">

                <div class="flex items-start justify-between mb-5">
                    <div class="w-9 h-9 rounded-xl bg-[#1f2937] border border-[#2e3841] flex items-center justify-center">
                        <svg class="w-4 h-4 text-[#2e3841]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                            <path d="{{ $cat['icon'] }}"/>
                        </svg>
                    </div>
                ...
                <h3 class="text-sm font-bold text-[#697d91] mb-2">{{ $cat['label'] }}</h3>
                <p class="text-xs text-[#2e3841] leading-relaxed mb-5">{{ $cat['desc'] }}</p>

                <div class="space-y-1.5">
                    @foreach($cat['signals'] as $sig)
                    <div class="flex items-center gap-2">
                        <div class="w-1 h-1 rounded-full bg-[#2e3841] flex-shrink-0"></div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="rounded-2xl border border-[#1f2937] bg-[#17181c] p-6 transition-all duration-500"
                ...
                <div class="flex items-center justify-between mb-5">
                    <div class="w-9 h-9 rounded-xl bg-[#0093fd]/10 border border-[#0093fd]/20 flex items-center justify-center">
                        <svg class="w-4 h-4 text-[#0093fd]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                            <path d="{{ $step['icon'] }}"/>
                        </svg>
                    </div>
                    <span class="font-mono text-xs font-bold text-[#0093fd]/40">{{ $step['step'] }}</span>
                </div>
